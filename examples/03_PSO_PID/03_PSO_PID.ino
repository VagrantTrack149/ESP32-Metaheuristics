...
float simularPID(const std::vector<float>& k) {
    ...
    for (int i = 0; i < 800; ++i) {
        ...
        // Planta de primer orden + saturación
...
void setup() {
    ...
    // Kp ∈ [0,25], Ki ∈ [0,25], Kd ∈ [0,25]
    Bounds limites;
    limites.lower = {0.0f, 0.0f, 0.0f};
    limites.upper = {25.0f, 25.0f, 25.0f};

    ParticleSwarm pso(limites, simularPID, TAM_POBLACION);

    Serial.println("Auto-tuning PID con PSO...");
    pso.ejecutar(Gen, [](int it, int total, const Agente& mejor) {
        if ((it % 10 == 0) || (it == total - 1)) {
            Serial.printf("Iter %d/%d | IAE: %.4f\n", it + 1, total, mejor.fitness);
        }
    });

    const auto& g = pso.mejor().posicion;
    Serial.printf("\nKp=%.4f  Ki=%.4f  Kd=%.4f\n", g[0], g[1], g[2]);
