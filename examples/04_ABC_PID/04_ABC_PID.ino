...
void setup() {
    ...
    ArtificialBeeColony abc(limites, simularPID, TAM_POBLACION, 15);

    Serial.println("Auto-tuning PID con ABC...");
    abc.ejecutar(Gen, [](int it, int total, const Agente& mejor) {
        if ((it % 10 == 0) || (it == total - 1)) {
            Serial.printf("Iter %d/%d | IAE: %.4f\n", it + 1, total, mejor.fitness);
        }
    });

    const auto& g = abc.mejor().posicion;
    Serial.printf("\nKp=%.4f  Ki=%.4f  Kd=%.4f\n", g[0], g[1], g[2]);
